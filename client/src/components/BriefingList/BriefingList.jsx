...
const BriefingList = () => {
    ...
    const handleDeleteBriefing = async (briefingId) => {
        try {
            ...
            setState(s => ({
                ...s,
                briefings: s.briefings.filter(briefing => briefing._id !== briefingId)
            }));
        } catch (error) {
            console.error(error);
        }
    ...
    const handleEditBriefing = async () => {
        try {
            ...
            const data = await updateBriefingById(state.selectedBriefing._id, updatedBriefing);
            setState(s => ({
                ...s,
                briefings: s.briefings.map(b => b._id === data._id ? data : b),
                showModal: false
            }));
        } catch (error) {
            console.error(error);
            ...
            {state.briefings.map((briefing) => (
                <div key={briefing._id} className="briefing-item">
                    <div className="briefing-box">
                        ...
                        <MdSettings className="options-toggle" onClick={() => handleToggleOptions(briefing._id)} />
                        <button className="see-more-button" onClick={() => navigate(`/briefing/${briefing._id}`)}>
                    ...
                    <form onSubmit={(e) => {
                        ...
                        <label>
                            <input
                                ...
                                onChange={(e) => setState({ ...state, editedClientName: e.target.value })}
                            />
                        </label>
                        <label>
                            <textarea
